...
- **搜索历史** — 自动记录集市搜索（POE2），国服 / 国际服数据隔离。
- **收藏管理** — 收藏 / 文件夹 / 拖拽重排 / LZ-String 压缩导入导出（国服 + 国际服）。
- **复制 PoB** — 在搜索结果行加「复制PoB」按钮，直接复制 Path of Building 可导入的英文物品文本（国服 + 国际服 POE2）。
...
> 所有功能均同时支持国服（`poe.game.qq.com`）与国际服（`www.pathofexile.com`），数据按服务器前缀独立隔离；国际服默认全部关闭，可在设置页手动开启（国际服功能未充分测试，不保证可用）。
...
设置页（右键插件图标 → 选项，或侧边栏 ⚙）可逐功能按国服 / 国际服分别开关、管理 PoB 词典；侧边栏标题旁的 `?` 按钮打开功能介绍页。
