...
`content/ctx.js` → `features/{history,favorites,pob-copy,view-mods}.js` → `content/shell.js`
...
`lib/lz-string.js` → `lib/dict-normalize.js` → `lib/dict-build.js` → `background/{storage,favorites,dict,poe2db}.js`
...
- 已有 type：历史 `SAVE/GET/DELETE/CLEAR_SEARCH_RECORD/HISTORY`；收藏 `SAVE/GET/DELETE/CLEAR_FAVORITE(S)`、`MOVE_TO_FOLDER/ROOT`、`ADD_TO_FOLDER`、`DELETE/RENAME_FOLDER`、`EXPORT/IMPORT_FOLDER`；词典 `DICT_STATUS/ENSURE/REFRESH/IMPORT`；词缀 `POE2DB_FETCH`；`OPEN_OPTIONS`。
...
`host_permissions`：`poe2db.tw`（查看词缀抓取）、`ninja.710421059.xyz`（词典原料）。`permissions` 含 `unlimitedStorage`（词典存 storage.local）。
